...
import json  # dcc.Store 사용을 위해 추가

# ==============================================================================
# 0. DB 설정 및 데이터 로드 (GRAPH+KAKAO6.py의 0번 섹션)
# ==============================================================================

# ⚠️ DB 설정 정보: 사용자 환경에 맞게 수정 필요
DB_CONFIG = {
    'user': 'daejeon',
    'password': 'daejeon',
    'host': '192.168.0.2',
...
# ==============================================================================
# 1. 데이터 전처리 및 옵션 생성 (GRAPH+KAKAO6.py의 1번 섹션)
# ==============================================================================
...
GRAPH_TYPE_OPTIONS = [
    {'label': '선 그래프 (Line)', 'value': 'line'},
    {'label': '수직 막대 (Bar)', 'value': 'bar'},
]


# ==============================================================================
# 2. Plotly 그래프 생성 함수 (GRAPH+KAKAO6.py의 2번 섹션)
# ==============================================================================
...
def plot_interactive_gradio_1_2(house_type, energy_metric_name, year_selection_list, start_month, end_month,
    ...
    fig1 = plot_func(df_combined_line,
                     ...
                     title=f'1. {plot_title_prefix} - {month_title} 월별 {energy_metric_name} **(단순 수치)**',
    ...
    fig2 = plot_func(df_combined_bar,
                     x='MONTH', y='VALUE', color='YEAR_GROUP',
                     title=f'2. {plot_title_prefix} - {month_title} 월별 {ENERGY_LABELS[per_grfa_metric]} **(연면적당 수치)**',
...
def plot_gu_comparison(house_type, energy_metric_name, gu_selection_list, year_selection, start_month, end_month,
    ...
    fig = plot_func(monthly_data, x='MONTH', y=base_metric, color='GU_NAME',
                    title=f'3. {plot_title_prefix} - 구별 ({", ".join(gu_selection_list)}) {year_label} {month_title} {energy_metric_name} 추이',
...
def plot_dong_comparison(house_type, energy_metric_name, gu_name, dong_selection_list, year_selection, start_month,
    ...
    fig = plot_func(monthly_data, x='MONTH', y=base_metric, color='LEGALDONG',
                    title=f'3-1. {plot_title_prefix} - {gu_name} 내 동 비교 ({len(all_selected_dongs)}개 항목) {year_label} {month_title} {energy_metric_name} 추이',
...
def plot_age_group_usage(energy_metric_name, house_type):
    ...
    fig = px.bar(df_age_avg, x='AGE_GROUP', y=energy_metric_key,
                 title=f'노후도 그룹별 평균 {energy_metric_name} 비교',
                 labels={'AGE_GROUP': '노후도 그룹', energy_metric_key: y_axis_title},
                 template='plotly_white', color_discrete_sequence=['#4CAF50'])
    fig.update_layout(
        title={
            # 'text'는 제목을 다시 설정할 때 사용되므로 생략하거나 기존 제목을 사용합니다.
            # 'font' 객체 내부에 'size'를 지정합니다. (단위: 픽셀)
            'font': {'size': 14}  # 예시: 폰트 크기를 20px로 설정
        }
    )
    return fig
...
def plot_age_group_per_grfa(energy_metric_name, house_type):
    ...
    fig = px.bar(df_age_avg, x='AGE_GROUP', y=per_grfa_metric,
                 title=f'노후도 그룹별 평균 {ENERGY_LABELS[per_grfa_metric]} 비교',
...
def plot_neighborhood_comparison(target_match_key, energy_metric_name):
    ...
    if not target_match_key: return px.bar(title=f"6. 주소 검색 후 비교할 건물을 선택해주세요.", height=400)
    target_building = df_data[df_data[JOIN_KEY] == target_match_key]
    if target_building.empty: return px.bar(title=f"6. 주소: {target_match_key} 에 해당하는 건물 데이터가 없습니다.")
    ...
    fig = px.bar(df_result, x='Category', y='Usage',
                 title=f"6. 건물 주소 '{target_match_key}' vs. 반경 100m 건물 평균 ({energy_metric_name})",
...
def plot_similarity_comparison(target_match_key, energy_metric_name):
    ...
    unit_type = ENERGY_UNIT_DETAILS[energy_metric_key]['type']
    target_match_key = str(target_match_key).strip()
    if not target_match_key: return px.bar(title=f"7. 주소 검색 후 비교할 건물을 선택해주세요.", height=400)
    target_building = df_data[df_data[JOIN_KEY] == target_match_key]
    if target_building.empty: return px.bar(title=f"7. 주소: {target_match_key} 에 해당하는 건물 데이터가 없습니다.")
    ...
    y_axis_title = f'평균 {unit_type} ({unit})'
    fig = px.bar(df_result, x='Category', y='Usage',
                 title=f"7. 건물 주소 '{target_match_key}' vs. 연면적/노후도 유사 건물 평균 ({energy_metric_name})",
                 labels={'Category': '비교 대상', 'Usage': y_axis_title},
...
# ==============================================================================
# 3. Dash 앱 초기화 및 레이아웃 구조 정의
# ==============================================================================
# 외부 스타일시트를 사용하여 디자인 통일
app = dash.Dash(__name__, external_stylesheets=['https://codepen.io/chriddyp/pen/bWLwgP.css'])
app.title = "에너지 데이터 통합 모듈"

# dcc.Location: URL 경로를 읽기 위한 컴포넌트
# dcc.Store: 주소 검색 결과를 모든 페이지에서 공유하기 위한 컴포넌트
app.layout = html.Div([
    dcc.Location(id='url', refresh=False),
    # 주소 검색 결과를 저장할 dcc.Store
    dcc.Store(id='shared-target-match-key', data=None),
    html.Div(id='page-content')
])


# ==============================================================================
# 4. 개별 레이아웃 함수 정의 (각 URL 경로별 화면 정의)
# ==============================================================================

# 1/2번 그래프 레이아웃 (경로: /graph/1-2)
def layout_graph_1_2():
    YEAR_OPTIONS_ROW1 = YEAR_OPTIONS_MULTI[:3]
    YEAR_OPTIONS_ROW2 = YEAR_OPTIONS_MULTI[3:]
    return html.Div(style={'padding': '15px'}, children=[
        html.H3('📈 1. 전체 주택 유형별/년도별 월별 추이', style={'textAlign': 'center', 'marginTop': '15px', 'color': '#007bff'}),
        html.Div(id='filters-1-2', style={'padding': '10px'}, children=[
            html.Div(style={'display': 'flex', 'gap': '10px'}, children=[
                ...
                html.Div(style={'width': '20%'}, children=[html.Label("에너지 지표 (단일 선택)"),
                                                           dcc.Dropdown(id='energy-metric-1-2',
                                                                        options=METRIC_OPTIONS_DASH, value='전기 사용량',
                                                                        clearable=False)]),
                ...
                html.Div(style={'width': '30%'}, children=[
                    dcc.Markdown("**년도 선택 (체크박스, 다중 선택)**"),
                    html.Div(className='button-checklist-scroll', children=[
                        dcc.Checklist(id='year-selection-checklist',
                                      options=[{'label': i, 'value': i} for i in YEAR_OPTIONS_ROW1], value=['전체 평균'],
                                      inline=True, className='button-checklist'),
                        html.Hr(style={'margin': '2px 0'}),
                        dcc.Checklist(id='year-selection-checklist-continued',
                                      options=[{'label': i, 'value': i} for i in YEAR_OPTIONS_ROW2], value=[],
                                      inline=True, className='button-checklist'),
                    ]),
                    dcc.Store(id='selected-years-store', data=['전체 평균']),
                ]),
            ]),
            html.Div(style={'display': 'flex', 'gap': '10px', 'marginTop': '10px'}, children=[
                html.Div(style={'width': '20%'}, children=[html.Label("시작 월"), dcc.Dropdown(id='start-month-1-2',
                                                                                            options=MONTH_OPTIONS_DASH,
        ...
        dcc.Graph(id='plot-1-output', style={'width': '100%', 'height': '350px', 'marginTop': '20px'}),
        dcc.Graph(id='plot-2-output', style={'width': '100%', 'marginTop': '20px', 'height': '350px'}),
    ])


# 3번 그래프 레이아웃 (경로: /graph/3)
def layout_graph_3():
    return html.Div(style={'padding': '15px'}, children=[
        html.H3('📍 2. 구별 상세 추이 분석 (다중 구 비교)', style={'textAlign': 'center', 'color': '#FF5722'}),
            ...
            html.Div(style={'display': 'flex', 'gap': '10px'}, children=[
                html.Div(style={'width': '18%'}, children=[html.Label("주택 유형"),
                                                           dcc.Dropdown(id='house-type-3', options=HOUSE_OPTIONS_DASH,
                                                                        value='전체 평균', clearable=False)]),
                html.Div(style={'width': '18%'}, children=[html.Label("에너지 지표"), dcc.Dropdown(id='energy-metric-3',
                                                                                              options=METRIC_OPTIONS_DASH,
                                                                                              value='전기 사용량',
                                                                                              clearable=False)]),
                ...
                html.Div(style={'width': '18%'}, children=[html.Label("년도 선택"), dcc.Dropdown(id='year-selection-3',
                                                                                             ...
                                                                                             value='전체 평균',
                                                                                             clearable=False)]),
                html.Div(style={'width': '18%'}, children=[
                    html.Label("구 선택 (다중 선택)"),
                    html.Div(className='button-checklist-scroll', children=[
                        dcc.Checklist(id='gu-checklist-3', options=GU_OPTIONS_DASH, value=[initial_gu_name],
                                      inline=True, className='button-checklist')]),
                ]),
            ]),
            html.Div(style={'display': 'flex', 'gap': '10px', 'marginTop': '10px'}, children=[
...
# 3-1번 그래프 레이아웃 (경로: /graph/3-1)
def layout_graph_3_1():
    return html.Div(style={'padding': '15px'}, children=[
        html.H3('📍 3. 동별 상세 추이 분석 (단일 구 내 다중 동 비교)', style={'textAlign': 'center', 'color': '#FF9800'}),
        html.Div(id='filters-3-1', style={'padding': '10px'}, children=[
            html.Div(style={'display': 'flex', 'gap': '10px'}, children=[
                html.Div(style={'width': '18%'}, children=[html.Label("주택 유형"),
                                                           dcc.Dropdown(id='house-type-3-1', options=HOUSE_OPTIONS_DASH,
                                                                        value='전체 평균', clearable=False)]),
                html.Div(style={'width': '18%'}, children=[html.Label("에너지 지표"), dcc.Dropdown(id='energy-metric-3-1',
                                                                                              options=METRIC_OPTIONS_DASH,
                                                                                              value='전기 사용량',
                                                                                              clearable=False)]),
                html.Div(style={'width': '18%'}, children=[html.Label("그래프 종류"),
                ...
                html.Div(style={'width': '18%'}, children=[html.Label("년도 선택"), dcc.Dropdown(id='year-selection-3-1',
                                                                                             ...
                                                                                             value='전체 평균',
                                                                                             clearable=False)]),
                html.Div(style={'width': '18%'}, children=[html.Label("구 선택 (단일)"),
                                                           dcc.Dropdown(id='gu-dropdown-3-1', options=GU_OPTIONS_DASH,
                                                                        value=initial_gu_name, clearable=False)]),
                html.Div(style={'width': '18%'}, children=[
                    html.Label("동 선택 (다중 선택)"),
                    html.Div(className='button-checklist-scroll', children=[dcc.Checklist(id='dong-checklist-3-1',
                                                                                          options=[
                                                                                              {'label': i, 'value': i}
                                                                                              for i in
                                                                                              initial_dong_choices],
                                                                                          value=['전체 동 평균'],
                                                                                          inline=True,
            ...
            html.Div(style={'display': 'flex', 'gap': '10px', 'marginTop': '10px'}, children=[
                html.Div(style={'width': '18%'}, children=[html.Label("시작 월"), dcc.Dropdown(id='start-month-3-1',
                                                                                            options=MONTH_OPTIONS_DASH,
                                                                                            value=1, clearable=False)]),
                html.Div(style={'width': '18%'}, children=[html.Label("종료 월"),
...
# 4/5번 그래프 레이아웃 (경로: /graph/4-5)
def layout_graph_4_5():
    ...
    return html.Div(style={'padding': '15px'}, children=[
        ...
        html.Div(style={'display': 'flex', 'gap': '10px', 'padding': '10px', 'justifyContent': 'flex-start'}, children=[
            html.Div(style={'width': '20%'}, children=[html.Label("에너지 지표"),
                                                       dcc.Dropdown(id='energy-metric-4-5', options=METRIC_OPTIONS_DASH,
                                                                    value='전기 사용량', clearable=False)]),
            html.Div(style={'width': '20%'}, children=[html.Label("주택 유형"),
                                                       dcc.Dropdown(id='house-type-4-5', options=HOUSE_OPTIONS_DASH,
                                                                    value='전체 평균', clearable=False)]),
        ]),
        html.Div(style={'display': 'flex', 'marginTop': '10px', 'gap': '10px'}, children=[
            html.Div(style={'width': '50%'}, children=[dcc.Graph(id='plot-4-output', style={'height': '400px'})]),
            html.Div(style={'width': '50%'}, children=[dcc.Graph(id='plot-5-output', style={'height': '400px'})]),
        ]),
    ])


# 6/7번 그래프 레이아웃 및 주소 검색 (경로: /graph/6-7)
def layout_graph_6_7():
    return html.Div(style={'padding': '15px'}, children=[
        html.H3('🔍 5. 개별 건물 비교 분석 (벤치마킹)', style={'textAlign': 'center', 'color': '#3F51B5'}),
        html.Div(style={'padding': '15px', 'border': '1px solid #ddd', 'borderRadius': '5px', 'marginBottom': '20px',
                        'backgroundColor': '#f9f9f9'}, children=[
            html.H4('🗺️ 대상 건물 주소 검색 (Kakao API)', style={'marginTop': '0', 'color': '#3F51B5'}),
            html.Div(style={'display': 'flex', 'gap': '10px'}, children=[
                html.Div(style={'width': '50%'}, children=[html.Label("검색할 주소/장소명 입력"),
                                                           dcc.Input(id='search-address-input', type='text',
                                                                     placeholder='예: 대전시청 또는 대전광역시 서구 둔산동 123-4',
                                                                     style={'width': '100%'})]),
                html.Div(style={'width': '20%'}, children=[
                    html.Label("검색 실행"),
                    html.Button('주소/장소 검색 및 MATCH_KEY 찾기', id='search-button', n_clicks=0,
                                style={'height': '38px', 'marginTop': '7px', 'backgroundColor': '#FFEB3B',
        ...
        html.Div(style={'display': 'flex', 'gap': '10px', 'padding': '10px', 'justifyContent': 'flex-start'}, children=[
            # dcc.Store와 연결될 컴포넌트
            html.Div(style={'width': '30%'}, children=[
                html.Label("대상 건물 주소 (MATCH_KEY)"),
                dcc.Input(id='target-match-key', type='text', value='', debounce=True,
                          placeholder='예: 서구 둔산동 1-1 (데이터에 존재하는 MATCH_KEY)'),
            ]),
            html.Div(style={'width': '20%'}, children=[html.Label("에너지 지표 (6/7번 공통)"),
                                                       dcc.Dropdown(id='energy-metric-6-7', options=METRIC_OPTIONS_DASH,
                                                                    value='전기 사용량', clearable=False)]),
        ]),
        html.H4('6. 특정 건물 vs. 반경 100m 건물 평균 비교', style={'marginTop': '30px', 'color': '#2E7D32'}),
        dcc.Loading(id="loading-6", type="circle", children=[dcc.Graph(id='plot-6-output', style={'height': '450px'})]),
        html.Hr(),
        html.H4('7. 특정 건물 vs. 연면적/노후도 유사 건물 평균 비교', style={'marginTop': '30px', 'color': '#388E3C'}),
        dcc.Loading(id="loading-7", type="circle", children=[dcc.Graph(id='plot-7-output', style={'height': '450px'})]),
    ])


# ==============================================================================
# 5. 페이지 이동 및 데이터 공유 콜백
# ==============================================================================
...
def display_page(pathname):
    if pathname == '/graph/1-2':
        return layout_graph_1_2()
    elif pathname == '/graph/3':
        return layout_graph_3()
    elif pathname == '/graph/3-1':
        return layout_graph_3_1()
    elif pathname == '/graph/4-5':
        return layout_graph_4_5()
    elif pathname == '/graph/6-7':
        return layout_graph_6_7()
    # 기본 경로 등
    return html.Div([html.H1('Dash 그래프 통합 서버', style={'textAlign': 'center'})], style={'padding': '50px'})


# 주소 검색 API 콜백 (결과를 dcc.Store에 저장)
@app.callback(
    [Output('shared-target-match-key', 'data'), Output('search-status-output', 'children')],
    [Input('search-button', 'n_clicks')],
    [dash.dependencies.State('search-address-input', 'value')]
)
def handle_address_search_and_store(n_clicks, search_query):
    if n_clicks is None or n_clicks == 0 or not search_query:
        # 초기 로드 시 또는 클릭 없을 시 업데이트 안함
        return dash.no_update, ""

    # GRAPH+KAKAO6.py의 주소 검색 및 가장 가까운 MATCH_KEY 찾는 로직
    ...
    if not df_valid_dist.empty:
        ...
        status_text = (
            ...
            f"가장 가까운 데이터 건물: **'{closest_match_key}'** (거리: {closest_distance:.2f}m) 로 지정되었습니다."
        )
        return closest_match_key, status_text  # MATCH_KEY를 dcc.Store에 저장
    else:
        status_text = f"⚠️ '{full_address}' 주변에 비교할 건물 데이터가 없거나 유효한 위/경도 정보가 부족합니다."
        return dash.no_update, status_text  # 결과 없을 시 dcc.Store 업데이트 안함


# dcc.Store 값을 MATCH_KEY 입력란에 동기화
@app.callback(
    Output('target-match-key', 'value'),
    [Input('shared-target-match-key', 'data')]
)
def sync_match_key(stored_match_key):
    # dcc.Store의 값이 None이 아니면 입력란을 업데이트합니다.
    if stored_match_key is not None:
        return stored_match_key
    return dash.no_update


# ==============================================================================
# 6. 그래프 업데이트 콜백 (GRAPH+KAKAO6.py의 4번 섹션)
# ==============================================================================

# 1. 체크리스트 두 개의 값을 합쳐서 그래프 콜백에 전달할 dcc.Store에 저장
@app.callback(
    Output('selected-years-store', 'data'),
    [Input('year-selection-checklist', 'value'),
     Input('year-selection-checklist-continued', 'value')]
)
def merge_checklist_values(checklist_row1, checklist_row2):
    checklist_row1 = checklist_row1 if checklist_row1 is not None else []
    checklist_row2 = checklist_row2 if checklist_row2 is not None else []
    selected_years = checklist_row1 + checklist_row2
...
if __name__ == '__main__':
    print("=" * 80)
    print("### 📊 Dash 통합 앱 (port:8051) 실행 중... ")
    print("   ➡️ 이 서버가 Flask 페이지의 모든 그래프 요청을 처리합니다.")
    print("=" * 80)
    # dev_tools_ui=False 설정은 <iframe>에서 불필요한 Dash UI를 제거합니다.
    app.run(host='0.0.0.0', debug=True, dev_tools_ui=False, port=8051)
